...
🚀 Live Demo & Links
⚡ Live Vercel Deployment (Primary / Highlighted): https://ondc-lock-decentralized-escrow-dispute-resolution-j4zinmk8a.vercel.app
🌐 Alternative Surge Web Link: https://ondclock-settlement.surge.sh
📱 Mobile Responsive Link: https://ondc-lock-decentralized-escrow-dispute-resolution-j4zinmk8a.vercel.app
...
The dashboard displays locked order escrow parameters. Users are prompted to connect their Stellar wallet at the top right:Buyer Dashboard

2. Stitch Custom Wallet Connection Modal
Clicking "Connect Wallet" opens our custom modal styled according to the Stitch design specifications (QR Mobile Sync on the left, Freighter/Albedo options on the right):Wallet Connection Modal

3. Freighter Wallet Connection Request Popup
Selecting Freighter initiates the extension trigger, asking the user to authorize connection to the ONDC-Lock application:Freighter Connection Request

4. Buyer Dashboard (Connected State)
Once connected, the wallet address is securely queried, truncated, and displayed at the top right, enabling escrow deployment:Buyer Dashboard Connected
...
Coordinate Representation: Latitudes and longitudes are scaled by 
10
7
10 
7
  (e.g., Delhi coordinates 28.7041 Lat, 77.1025 Lon are stored as integers 287041000 and 771025000 respectively).
Tolerance Boundary: Set to 
10
,
000
10,000 coordinate units, which equates to roughly 50–100 meters depending on latitude.
On-Chain Formula: 
dist_sq
=
(
...
dist_sq≤tolerance 
2
 , the rider is verified as physically present at the delivery target.
...
95.0
%
95.0% of the escrow amount.
Logistics Provider (Rider): 
2.5
%
2.5% of the escrow amount.
ONDC Platform / Protocol Fee: 
2.5
%
2.5% of the escrow amount.
...
Setup Wallet: Install the Freighter Extension or use Albedo. Switch your network settings to Stellar Testnet and fund your key with testnet XLM using the Stellar Friendbot Faucet.
Deploy Escrow (Buyer):
